<template>
  <el-container direction="vertical" style="height: 100%;width: 100%;padding: 0">
    <el-main class="msg-box" ref="msg-box">
      <div v-for="chatMessage in chatMessageList">
        <el-row v-if="chatMessage.adminid.toString()!==adminid.toString()" style="height: fit-content">
        ...
        <el-row v-else style="direction: revert">
          ...
          <el-col :span="23">
            <el-row style="font-size: 13px;line-height: 10px;text-align: right">{{ chatMessage.adminname }}</el-row>
...
export default {
  ...
  methods: {
    sendMessage() {
      let data = {adminid: this.adminid, content: this.inputText}
      this.$http.post("/admin/addTextMessage", data).then(res => {
        if (res.data === 'success') {
          this.sendMsg()
          this.inputText = "";
        } else {
    ...
    async joinRoom() {
      ...
      this.ws = new WebSocket(this.url + "admin/" + this.adminid + '/' + this.admin.name + '/' +'admin.jpeg');
    ...
    webscoketonmessage(value) {
      ...
      // else{
      //   message.sendType = attributes[0];
      //   message.adminid = attributes[1];
      //   message.adminname = attributes[2];
      //   message.photo = attributes[3];
      //   message.msgType = attributes[4];
      //   message.time = attributes[5];
      //   let len = 0;
      ...
      // if (message.adminname.toString() === this.adminid.toString()) {
      //   this.scrollBottom();
      // }
    },
    exitRoom() {
    ...
    flushChatMessageList() {
      this.$http.get('/admin/getChatMessageList').then(res => {
  ...
  created() {
    this.adminid = parseInt(window.sessionStorage.getItem("adminid"))
    this.$http.get("/admin/getAdminById", {params: {id: this.adminid}}).then(res => {
      this.admin = res.data
      this.joinRoom();
    })
    this.flushChatMessageList();
